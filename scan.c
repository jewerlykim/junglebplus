...
{ // 반복되는 삶.. 힘들다...
    ...
    { // 지웠을때 최소길이 만족이 안됨;;;;;;;;;;;;;;;;;;;;;고난 시작
        ...
        { // 부모가 있엉!
            int child_idx = 0;
            for (int i = 0; i <= curr->parent->key_count; i++)
            { // 나를 찾아줘..
        ...
        if ((curr->parent->key_count) < (N - N / 2 - 1))
        { // 부모 가 최소개수가 안돼...
...
void delete_node(NODE *curr, int curr_key, NODE **root)
{
    ...
    {                 // 존재한다;; 이것도 힘들어;;
        ...
        { // 자식이 없네???? 리프네??!!
            ...
            for (int j = idx; j < (tmp->key_count); j++)
            { // 땡겨주기 baby~~
...
void up(int key, NODE *curr, NODE **root)
{
    ...
    { // 부모가 있을 때
        ...
        for (int i = curr->parent->key_count; i > idx; i--)
        { // 부모 밀기
